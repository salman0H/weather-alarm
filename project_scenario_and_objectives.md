...
- **Enable Test Mode (`TEST_MODE=true`):** با روشن کردن این گزینه، سیستم به جای دریافت زنده داده‌ها از API، داده‌های کش‌شده و محلی (مانند `tests/fixtures/sample_alert.json`) را پردازش می‌کند. این قابلیت به ویژه برای توسعه و اجرای سریع تست پایپ‌لاین (مثلاً `tests/test_pipeline.py`) بسیار کاربردی است، زیرا مانع از مصرف سهمیه API (Rate Limiting) شده و امکان شبیه‌سازی کل فرآیند از ارزیابی تا ارسال پیام را فراهم می‌کند. این تنظیم به صورت Input در منوی اجرای دستی (Workflow Dispatch) گیت‌هاب اکشنز نیز قرار داده شده است.
- **Force a Mock Severe Alert (`MOCK_ALERT=true`):** با فعال‌سازی این حالت، یک بحران شدید هواشناسی (مانند تندباد خطرناک یا بارش شدید مندرج در فایل فیکسچر) به صورت مصنوعی در داده‌ها تزریق می‌شود (Inject). با این کار، مسیر اجرای کد به سمت تولید پیام حالت هشدار (Alert) هدایت می‌شود تا طراح سیستم بتواند ظاهر پیام هشدار، رنگ‌بندی‌ها، دکمه شیشه‌ای (Inline Keyboard) و مکانیزم Acknowledge را در تلگرام بدون نیاز به انتظار برای وقوع واقعی یک بحران جوی تست کند. این مورد نیز به عنوان یک Input در گیت‌هاب اکشنز قابل دسترسی است.
